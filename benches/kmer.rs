...
use dima_lib::{AlphabetType, CharacterValidator, ValidationMode};
...
fn make_protein_with_gaps(len: usize, gap_fraction: f64) -> Vec<u8> {
    ...
    (0..len)
        .map(|i| {
            if gap_interval > 0 && i % gap_interval == 0 {
                b'-'
            } else {
...
fn bench_sliding_window_protein(c: &mut Criterion) {
    ...
    let validator =
        CharacterValidator::with_options(AlphabetType::Protein, ValidationMode::default(), true);
...
fn bench_sliding_window_nucleotide(c: &mut Criterion) {
    ...
    let validator =
        CharacterValidator::with_options(AlphabetType::Nucleotide, ValidationMode::default(), true);
