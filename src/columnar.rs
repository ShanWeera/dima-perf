        ...
        match metadata {
            Some(meta_map) => {
                for (field_idx, field_name) in self.field_names.iter().enumerate() {
                    let value = meta_map.get(field_name)
...
mod tests {
    ...
    fn test_vectorized_operations() {
        ...
        let usa_matches = ColumnarOperations::compare_field_values_vectorized(country_column, "USA");
    ...
    fn test_bulk_operations() {
        ...
        // Bulk update
        let updates = vec![(0, "Updated_USA".to_string()), (1, "Updated_Canada".to_string())];
        columnar.bulk_update_field("Country", &updates);
        
        let updated_meta = columnar.get_sequence_metadata(0).unwrap();
        assert_eq!(updated_meta.get("Country"), Some(&"Updated_USA".to_string()));
    }
    ...
    fn test_performance_comparison() {
        ...
        for i in 0..10000 {
            let mut map = HashMap::new();
            map.insert("Date".to_string(), format!("2023-{:02}-{:02}", (i % 12) + 1, (i % 28) + 1));
            map.insert("Country".to_string(), ["USA", "Canada", "Mexico", "Brazil"][i % 4].to_string());
            map.insert("Species".to_string(), ["Human", "Mouse", "Rat"][i % 3].to_string());
