    ...
    pub fn build_from_column(
        ...
        for (seq_idx, value_opt) in column.iter().enumerate() {
            if let Some(value) = value_opt {
                value_indices.entry(Arc::clone(value))
...
impl CompositeIndex {
    ...
    pub fn query_and(&mut self, field_queries: &[(&str, &str)]) -> IndexBitmap {
        ...
        if self.combination_cache.len() < self.max_cache_size {
            self.combination_cache.insert(cache_key, final_result.clone());
        }
    ...
    fn create_cache_key(&self, field_queries: &[(&str, &str)], operation: &str) -> String {
        let mut key_parts: Vec<String> = field_queries.iter()
...
impl MetadataIndexManager {
        ...
        let indices: Vec<(String, Arc<InvertedIndex>)> = field_names.par_iter()
            .filter_map(|field_name| {
                columnar_metadata.get_field_column(field_name)
                    .map(|column| {
                        let index = InvertedIndex::build_from_column(
                            field_name.clone(),
                            column
                        );
    ...
    fn build_composite_indices(&mut self) {
        ...
        if self.config.priority_fields.len() >= 2 {
            ...
            self.composite_indices.insert("priority_fields".to_string(), composite);
        }
    }
    ...
    pub fn get_comprehensive_stats(&self) -> IndexManagerStats {
        ...
        for (field_name, index) in &self.field_indices {
            stats.field_stats.insert(field_name.clone(), index.get_stats());
        }
...
mod tests {
    ...
    fn test_large_dataset_performance() {
        ...
        let countries = ["USA", "Canada", "Mexico", "Brazil", "UK", "Germany", "France", "Japan"];
        ...
        for i in 0..10000 {
            ...
            map.insert("Country".to_string(), countries[i % countries.len()].to_string());
            map.insert("Species".to_string(), species[i % species.len()].to_string());
        ...
        assert!(speedup > 2.0, "Expected significant speedup, got {:.2}x", speedup);
