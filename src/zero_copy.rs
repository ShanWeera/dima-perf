...
pub fn parse_header_zero_copy(
    ...
    ZERO_COPY_PARSER.with(|parser| parser.parse_zero_copy(header, format, fill_na))
}
...
mod tests {
    ...
    fn test_batch_processing() {
        ...
        assert_eq!(
            results[1].get("condition"),
            Some(&"condition_B".to_string())
        );
        assert_eq!(
            results[2].get("replicate"),
            Some(&"replicate_3".to_string())
        );
    }
    ...
    fn test_error_handling() {
        ...
        let result = parser.parse_zero_copy("sample1|condition_A", fields, "Unknown");
