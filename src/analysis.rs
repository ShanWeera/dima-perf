...
use crate::io::{
    ...
    get_kmers_and_headers_encoded_columnar_validated,
...
/// Production-grade analysis with columnar metadata storage and validated character handling
/// 
/// This is the recommended function for analysis. It uses whitelist-based character
/// validation to ensure only valid biological sequences are processed.
pub fn get_results_objs_columnar_validated(
    path: String,
    kmer_length: usize,
...
) -> (Results, Option<ValidationStats>) {
    ...
    let early_pb = if show_progress {
        ...
        pb.set_message("Reading FASTA and building k-mer matrix (columnar, validated)...");
        pb.enable_steady_tick(std::time::Duration::from_millis(1));
        Some(pb)
    } else { None };

    let (encoded_kmers, columnar_headers, sequence_count, is_protein, validation_stats) = 
        get_kmers_and_headers_encoded_columnar_validated(
            &path,
            &kmer_length,
...
/// Production-grade analysis with validated character handling
/// 
/// This is the recommended function for analysis. It uses whitelist-based character
/// validation to ensure only valid biological sequences are processed.
pub fn get_results_objs_validated(
    path: String,
    kmer_length: usize,
    support_threshold: usize,
    query_name: String,
    ...
    let early_pb = if show_progress {
        let pb = indicatif::ProgressBar::new_spinner();
        pb.set_message("Reading FASTA and building k-mer matrix (validated)...");
        pb.enable_steady_tick(std::time::Duration::from_millis(1));
        Some(pb)
    } else { None };

    let (encoded_kmers, headers, sequence_count, is_protein, validation_stats) = get_kmers_and_headers_validated(
        &path,
        &kmer_length,
        header_format.as_ref(),
        header_fillna.as_ref(),
...
/// Production-grade analysis with columnar metadata storage (legacy)
/// 
/// DEPRECATED: This function uses a blacklist approach which may allow invalid
/// characters. Use get_results_objs_columnar_validated instead.
#[deprecated(
    since = "2.0.0",
    note = "Use get_results_objs_columnar_validated for robust whitelist-based validation"
)]
...
/// Legacy analysis function with blacklist approach
/// 
/// DEPRECATED: This function uses a blacklist approach which may allow invalid
/// characters. Use get_results_objs_validated instead.
#[deprecated(
    since = "2.0.0",
    note = "Use get_results_objs_validated for robust whitelist-based validation"
)]
